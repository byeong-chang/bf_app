...
export default function SearchLocationScreen({ route, navigation }) {
  ...
  const handleSearch = async () => {
    const data = {
      hobbyId: hobby,
      locationId: locationCategory
    };
    const res = await api_post.searchLocation(data);
    setLocations(res);
  };
    ...
    <View style={{ alignItems: "center", justifyContent: "center" }}>
      <ScrollView>
        ...
        >
          <View style={{ width: "70%" }}>
            <SelectList
              placeholder="검색할 지역을 선택해주세요"
              searchPlaceholder="지역"
              setSelected={(val) => {
                const index =
                  loactionData.findIndex((key) => key.value == val) + 1;
                console.log(val + "의 키값은 : " + index);
                setLocationCategory(index);
              }}
              data={loactionData}
              save="value"
              dropdownShown={false}
            ...
            {/* hobby */}
            <SelectList
              placeholder="검색할 취미를 선택해 주세요"
              searchPlaceholder="취미"
              setSelected={(val) => {
                const index =
                  hobbyData.findIndex((key) => key.value == val) + 1;
                console.log(val + "의 키값은 : " + index);
                setHobby(index);
              }}
              data={hobbyData}
