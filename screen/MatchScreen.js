...
const MatchScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      const detailData = await api_get.recruitmentDetail(route.params.recruitId);
      setData(() => detailData);
    };      
    getData();
  }, []);
  ...
  function ChattingBlock (props) {
    ...
    return (
      <View>
        <ChattingBox key={`ChattingBox-${props.chatMain.currentChatId}`} chat={props.chatMain}/>
        <View style={{marginLeft: 20}}>
          {props.chatSub.map((chat) => 
            <ChattingBox key={`ChattingBox-${chat.currentChatId}`}chat={chat}/>
          )}
        </View>
        <View style={{width: "30%"}}>
