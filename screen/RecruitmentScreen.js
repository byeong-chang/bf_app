...
import { StyleSheet, Text, TextInput, View, Button, ScrollView, TouchableWithoutFeedback } from "react-native";
...
function PostingBox({title, location, id, date, content}) {
  return (
    <View
      style={{ borderRadius: 20, padding: 8, backgroundColor: "white", margin: 4}}
    >
...
export default function RecruitScreen({ route, navigation }) {
  ...
  const postings = data.map((po) => (
    <PostingBox key={`postingbox-${po.id}`}
      title={po.title}
